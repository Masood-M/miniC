...
def scanning(script_path):	
#	script_path = os.path.dirname(os.path.abspath( __file__ ))
	start_time = time.time()
	currentpathtmp=script_path+'/tmp/'
	os.chdir(currentpathtmp)
	with open(script_path+'/scanlogs/Clam-report.log', "w") as f:
		...
		for line in iter(process.stdout.readline, ''):
			...
			f.write(line)
		f.write("\nclamscan -r --stdout --infected --scan-html=yes --scan-pdf=yes --scan-archive=yes -i --algorithmic-detection=yes " + currentpathtmp + "\n\n")
